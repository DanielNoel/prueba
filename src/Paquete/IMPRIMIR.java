...
public class IMPRIMIR {
 ...
 public void ImprimirsinParametros(String Titu,String NRpt) {
  try {
       ...
       parame.put("","");      
       JasperPrint prt = JasperFillManager.fillReport(rta,parame,cconn.cnx);
       int n = prt.getPages().size();                 
       if (n > 0) {
        if(JOptionPane.showConfirmDialog(null,"Deseas Previsualizar primero","Confirmar"
         ,0)==0){
         JasperViewer JsperView = new JasperViewer(prt, false);JsperView.setTitle(Titu);
 ...
 public void Imprimircon1Parametro(String Titu,String NRpt,String prm1,String vlrprm1) {
  try { Map parame = new HashMap();
 ...
 public void Imprimircon2Parametros(String Titu,String NRpt,String prm1,String prm2,
         String vlrprm1,String vlrprm2) {
  try {Map parame = new HashMap();
       rta = System.getProperties().getProperty("user.dir") + "/src/Reportes/" + NRpt+".jasper";
       parame.put(prm1,vlrprm1);parame.put(prm2,vlrprm2);        
       JasperPrint prt = JasperFillManager.fillReport(rta,parame,cconn.cnx);int n = prt.getPages().size();                 
       if (n > 0) {
 ...
 String vlr1,String vlr2,String igv,String mto) {
  try {
    ...
    public void Impresion(String NRpt,String titulo) {
        try {
            Map parame = new HashMap();
            rta = System.getProperties().getProperty("user.dir") + "/src/Reportes/" + NRpt + ".jasper";
            parame.put("", "");            
    ...
    public void imprimir(String nombrerep,String titulo,Map m){
        ...
        JasperPrint prt = JasperFillManager.fillReport(rta, m, cconn.cnx);
            int n = prt.getPages().size();
            if (n > 0) {
                JasperViewer JsperView = new JasperViewer(prt, false);
                JsperView.setTitle(titulo);
